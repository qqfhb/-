...
x=C1(:,1);y=C1(:,2);
sj=[x,y];d1=[70,40];
sj=[sj;d1];                %the coordinate including the start-end nodes
N=73;                        % the number of the cities
...
%»æÍ¼
 figure
for i=1:N 
    plot([sj(temp1(i),1),sj(temp1(i+1),1)],[sj(temp1(i),2),sj(temp1(i+1),2)],'ms-','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','g');  
    text(sj(temp1(i),1),sj(temp1(i),2),['  ',int2str(temp1(i))]);  
    text(sj(p(i+1),1),sj(p(i+1),2),['  ',int2str(p(i+1))]);  
    hold on;  
end  
plot([sj(temp1(N+1),1),sj(temp1(1),1)],[sj(temp1(N+1),2),sj(temp1(1),2)],'ms-','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','r');  
 title(['ÓÅ»¯×î¶Ì¾àÀë£º',num2str(Shortest_Len(end))]);
Shortest_Len(end)
